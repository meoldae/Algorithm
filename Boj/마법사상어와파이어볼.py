...
for _ in range(k):
    ...
    board = [[[0, 0, 0, 0, 0]]*n for _ in range(n)]
    fireballs = set()
    ...
    for j in range(len(queue)):
        ...
        board[nx][ny] = [board[nx][ny][0]+m, board[nx][ny][1]+s, board[nx][ny][2]+d, board[nx][ny][3]+1, board[nx][ny][4]+(d%2)]
        # 파이어볼 있는곳 좌표 저장 
        fireballs.add((nx, ny))
    
    # 이동 후
    for x, y in fireballs:
        m, s, d, count, temp_d = board[x][y]
        ...
        if count == 1:
            ...
            # 질량 0이면 소멸
            if m // 5:        
                if (temp_d == count) or (temp_d==0): # Directions are same
